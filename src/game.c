...
void particleDraw(Thing* t) {
    ...
    if (template.colorCount > 0) {
        ...
        if (template.colorCount > 1) {
            ...
            idx = clamp((int)floorf(percentage * template.colorCount), 0, template.colorCount - 1);
        }
        DrawEllipse((int)TO_FLOAT_16(t->subX), (int)TO_FLOAT_16(t->subY),
                    TO_FLOAT_8(t->scaleX), TO_FLOAT_8(t->scaleY),
                    hex2Color(palette[idx]));
    }
}

void alienUpdate(Thing *t) {
    if (t->alarms[2] <= 0) t->alarms[2] = 255;
